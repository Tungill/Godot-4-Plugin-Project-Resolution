...
var landscape = false
var hide_larger_resolution = true
...
#	print("current_res: " + current_res)
	...
	# Add Button settings
	var _buttons = ["Fullscreen", "Play Auto On Change", "Play Auto Current Scene", "Multistart", "Hide Larger Resolution"]
	for button in _buttons:
		isPluginEnabled = false
		if button == "Fullscreen" and current_fullscreen == 3:
			isPluginEnabled = true
		if button == "Play Auto On Change" and play_on_change == true:
			isPluginEnabled = true
		if button == "Play Auto Current Scene" and play_current_scene == true:
			isPluginEnabled = true
		if button == "Multistart" and multistart == true:
			isPluginEnabled = true
		if button == "Hide Larger Resolution" and hide_larger_resolution == true:
			isPluginEnabled = true
	...
	_plugins_menu.add_separator("DEKSTOP");
	_menu_items_idx += 1
	
	var _resolutions = ["Native", "2560x1600", "2560x1440", "1920x1200", "1920x1080", "1600x1200", "1280x1024", "1280x720", "1024x768", "800x600", "720x405", "640x480", "640x360","320x240", "0x0"]
	for resolution in _resolutions:
		...
		if resolution != "Native":
			...
			if hide_larger_resolution == true:
	...
	var _widescreen_resolutions = ["5120x1440", "3840x2160", "3840x1080", "3440x1440", "2560x1080", "2560x720", "1920x540", "1280x360"]
	...
	# Add handheld console resolutions
	_plugins_menu.add_separator("HANDLED CONSOLE");
	_menu_items_idx += 1
	
	_plugins_menu.add_separator("Steam Deck");
	_menu_items_idx += 1
	var _steam_deck_resolutions = ["1280x800"]
	for resolution in _steam_deck_resolutions:
		isPluginEnabled = false
		if resolution == current_res:
			isPluginEnabled = true
	...
	_plugins_menu.add_separator("Nintendo Switch");
	_menu_items_idx += 1
	var _nintendo_switch_resolutions = [ "1280x720"]
	for resolution in _nintendo_switch_resolutions:
		isPluginEnabled = false
		if resolution == current_res:
			isPluginEnabled = true
...
func _set_resolution(id) -> void:
	...
	elif item_name == "Play Auto On Change":
		play_on_change = !play_on_change
		print("Play Auto On Change: "+str(play_on_change))
	elif item_name == "Play Auto Current Scene":
		play_current_scene = !play_current_scene
		print("Play Auto Current Scene: "+str(play_current_scene))
	...
	elif item_name == "Hide Larger Resolution":
